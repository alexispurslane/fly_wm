...
void WindowManager::Run() {
  ...
  XParseColor(display_, DefaultColormap(display_, 0), "#00bfff", &xcolor);;
  xcolor.flags = DoRed | DoGreen | DoBlue;
  XAllocColor(display_, DefaultColormap(display_, XDefaultScreen(display_)), &xcolor);

  XSetWindowBackground(display_, root_, xcolor.pixel);
...
void WindowManager::Frame(Window w) {
  ...
  const unsigned long BORDER_COLOR = 0xdddddd;
